...
def fetch_log_file(ftp_settings, last_position):
    try:
        ftp = ftplib.FTP()
        ...
        if last_position.get(filename):
            ...
            if filesize > last_position[filename]:
                ...
                with open(local_filename, "r", encoding="utf8") as file:
...
def print_settings(settings):
    ...
    for key, value in settings.items():
        if key == "ftp":
            print("FTP:")
            for ftp_key, ftp_value in value.items():
                print(f"  {ftp_key}: {ftp_value}")
        else:
            print(f"{key}: {value}")
...
def main():
    ...
    while True:
        log_line = fetch_log_file(ftp_settings, last_position)
        if log_line:
            ...
            print("Failed to fetch the log line from FTP.")
        time.sleep(1) 
